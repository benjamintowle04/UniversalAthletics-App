...
// Safe getter for callers that want to attempt to obtain the initialized
// Auth instance at runtime. Some builds/environments can throw when calling
// getAuth() eagerly ("Component auth has not been registered yet").
// Callers can use this function and guard when it returns undefined.
export function getFirebaseAuthSafe(): Auth | undefined {
  ...
  try {
    ...
    try {
      ...
      console.error('[DIAG] backend getFirebaseAuthSafe failed to initialize auth', (err as any)?.stack ? (err as any).stack : err);
    } catch (e) {}
    return undefined;
  }
}
