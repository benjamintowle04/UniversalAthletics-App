...
const RequestASession = ({navigation, route}: RequestASessionProps) => {
  ...
  useEffect(() => {
    console.log("Profile Data recieved in Request a session", route.params)
  });
  ...
  // Web-specific date/time input handlers
  const handleWebDateChange = (option: number, value: string) => {
    const selectedDate = new Date(value);
    if (option === 1) {
      setDateTime1(prev => ({ ...prev, date: selectedDate }))
    } else if (option === 2) {
      setDateTime2(prev => ({ ...prev, date: selectedDate }))
    } else if (option === 3) {
      setDateTime3(prev => ({ ...prev, date: selectedDate }))
    }
  }

  const handleWebTimeChange = (option: number, value: string) => {
    const [hours, minutes] = value.split(':');
    const selectedTime = new Date();
    selectedTime.setHours(parseInt(hours), parseInt(minutes), 0, 0);
    
    if (option === 1) {
      setDateTime1(prev => ({ ...prev, time: selectedTime }))
    } else if (option === 2) {
      setDateTime2(prev => ({ ...prev, time: selectedTime }))
    } else if (option === 3) {
      setDateTime3(prev => ({ ...prev, time: selectedTime }))
    }
  }
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      const sessionRequestData = {
        ...
        receiverType: recipientType.toUpperCase(),
      ...
      if (recipientType === 'COACH') {
        console.log("Recipient type is coahc, creating member-to-coach session request")
        response = await createMemberToCoachSessionRequest(sessionRequestData)
      } else {
        console.log("Recipient type is member, creating coach-to-member session request")
      ...
      console.log("Updated user data, navigating back to the stack")
  ...
  // Web-specific date/time input component
  const renderWebDateTimeInput = (option: number) => {
    ...
    return (
      <View key={option} className="mb-4">
        ...
        <View className="flex-row justify-between mb-3">
          {/* Date Input */}
          <View className="flex-1 mr-2 p-4 rounded-lg border border-gray-200 bg-white">
            <View className="flex-row items-center">
              ...
              <View className="ml-3 flex-1">
                ...
                <input
                  type="date"
                  value={formatDateForInput(dateTimeState.date)}
                  onChange={(e) => handleWebDateChange(option, e.target.value)}
          ...
          {/* Time Input */}
          <View className="flex-1 ml-2 p-4 rounded-lg border border-gray-200 bg-white">
            <View className="flex-row items-center">
              ...
              <View className="ml-3 flex-1">
                ...
                <input
                  type="time"
                  value={formatTimeForInput(dateTimeState.time)}
                  onChange={(e) => handleWebTimeChange(option, e.target.value)}
                  style={{
                    border: 'none',
                    outline: 'none',
                    background: 'transparent',
      ...
      <ScrollView className="flex-1 bg-gray-50">
        <View className="p-4">
            ...
            <View className="flex-row items-center">
              ...
              <View className="flex-1">
                ...
                <Text className="text-gray-500 text-base mt-1">
                  Sending session request to {recipientFirstName} {recipientLastName}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View className="bg-white rounded-lg p-6 mb-6 shadow-sm border border-gray-100">
            ...
            {/* Render appropriate date/time picker based on platform */}
            {Platform.OS === 'web' ? (
              <>
                {renderWebDateTimeInput(1)}
                {renderWebDateTimeInput(2)}
                {renderWebDateTimeInput(3)}
              </>
            ) : (
              <>
